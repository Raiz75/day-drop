/* AI-CONTEXT-NOTE:{"R":"Category-aware progress dots with milestone dividers.","IDD":[{"?":"Dots are grouped by category; dividers appear between groups; current category gets a highlight ring."}],"A":["components/journal/JournalWizard.tsx"],"AB":["lib/journal/steps.ts CATEGORIES, STEPS"],"E":["npm run build","npm test"]} */
"use client";
...
export function ProgressDots({ currentStepIndex }: ProgressDotsProps) {
  ...
  return (
    <div className="flex items-center gap-1" aria-label={`step ${currentStepIndex + 1} of ${STEPS.length}`}>
      {groups.map((group, gi) => {
